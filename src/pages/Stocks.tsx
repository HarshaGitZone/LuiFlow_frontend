...
type PortfolioAnalytics = {
  topPerformers: PerfStock[]
  worstPerformers: PerfStock[]
  portfolioDistribution: DistributionStock[]
}
...
const Stocks: React.FC = () => {
  ...
  const searchStocks = async (query: string) => {
    ...
    try {
      const response = await api.get(`/api/stocks/search?keywords=${encodeURIComponent(query)}`)
  ...
  const editHolding = (holding: Holding) => {
    ...
    setFormData({
      ...
      buyDate: holding.buyDate ? holding.buyDate.split('T')[0] : '',
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col lg:flex-row lg:justify-between lg:items-center gap-4">
        <div>
          <h1 className="text-xl sm:text-2xl font-bold text-gray-900 dark:text-white">Stock Portfolio</h1>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 sm:gap-3 w-full lg:w-auto">
          ...
          <button
            ...
            className="flex items-center justify-center gap-2 px-3 sm:px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 disabled:opacity-50 transition-colors text-sm sm:text-base"
          >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4">
        ...
        <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow">
          <div className="flex items-center justify-between">
            ...
            <PieChart
              className={portfolioStats.totalPnLPercentage >= 0 ? 'text-green-500' : 'text-red-500'}
              size={24}
            />
          </div>
        </div>
      ...
      {showAddModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-3 sm:p-4">
          <div className="bg-white dark:bg-gray-800 rounded-lg p-4 sm:p-6 w-full max-w-md max-h-[92vh] overflow-y-auto">
      ...
      {showStockDetails && selectedStock && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-3 sm:p-4">
          <div className="bg-white dark:bg-gray-800 rounded-lg p-4 sm:p-6 w-full max-w-2xl max-h-[92vh] overflow-y-auto">
            <div className="flex justify-between items-center mb-4">
      ...
      {showCharts && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-3 sm:p-4">
          <div className="bg-white dark:bg-gray-800 rounded-lg p-4 sm:p-6 w-full max-w-6xl max-h-[92vh] overflow-y-auto">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <PortfolioChart
                ...
                data={analytics?.topPerformers
                  ?.concat(analytics?.worstPerformers || [])
                  ?.map(stock => ({
                    symbol: stock.symbol,
            ...
            <div className="mt-6 grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div
                className={`${portfolioStats.totalPnL >= 0 ? 'bg-green-50 dark:bg-green-900/20' : 'bg-red-50 dark:bg-red-900/20'} p-4 rounded-lg`}
              >
                <h4
                  className={`text-sm font-medium ${portfolioStats.totalPnL >= 0 ? 'text-green-900 dark:text-green-300' : 'text-red-900 dark:text-red-300'} mb-1`}
                >
                  Total P&L
                </h4>
                <p
                  className={`text-2xl font-bold ${portfolioStats.totalPnL >= 0 ? 'text-green-900 dark:text-green-300' : 'text-red-900 dark:text-red-300'}`}
                >
      ...
      {showAnalytics && analytics && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-3 sm:p-4">
          <div className="bg-white dark:bg-gray-800 rounded-lg p-4 sm:p-6 w-full max-w-4xl max-h-[92vh] overflow-y-auto">
            <div className="flex justify-between items-center mb-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div>
                ...
                <div className="space-y-2">
                  {analytics.topPerformers.map((stock) => (
                    <div key={stock._id || stock.symbol} className="flex justify-between items-center p-3 bg-green-50 dark:bg-green-900/20 rounded-lg">
                      <div>
                        <div className="font-medium text-gray-900 dark:text-white">{stock.symbol}</div>
                        <div className="text-sm text-gray-600 dark:text-gray-400">{stock.name}</div>
                      </div>
              ...
              <div>
                ...
                <div className="space-y-2">
                  {analytics.worstPerformers.map((stock) => (
                    <div key={stock._id || stock.symbol} className="flex justify-between items-center p-3 bg-red-50 dark:bg-red-900/20 rounded-lg">
                      <div>
                        <div className="font-medium text-gray-900 dark:text-white">{stock.symbol}</div>
                        <div className="text-sm text-gray-600 dark:text-gray-400">{stock.name}</div>
                      </div>
